...
const fs = require('fs-extra');  // To manage File System
...
program
  ...
  .action(function (appname, options) { // Function to run when the command is executed
    ...
    // Create the bin folder and a basic www file for server starting
    const binFile = `
      #!/usr/bin/env node
      ...
      function normalizePort(val) {
        ...
        if (isNaN(port)) return val; // If not a number, return the value as-is
        if (port >= 0) return port; // If it is a valid port, return it
        return false; // If it is an invalid port, return false
      }
    `;

    // Write the www file to the bin folder to make it executable
